...
if st.button('프로필 생성'):
    if not name or not organization or not position:
        ...
        payload = {
            ...
            "messages": [
                {"role": "user", "content": f"""
사용자가 제공한 이름, 소속, 직위 정보를 기반으로 검색된 정보만 활용하여 대상 인물의 정형화된 한글 프로필을 생성한다. 프로필은 크게 기본사항과 인물평으로 구성되며, 워드 기준 총 2페이지로 분량을 맞춘다.
...
아래 웹 검색 정보를 적극 활용하여 프로필을 생성하도록 한다. 
"""}
            ],
            "max_tokens": 500
        }
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                profile_text = response_json.get('choices', [{}])[0].get('message', {}).get('content', "프로필 생성에 실패했습니다.")
        except Exception as e:
